...
func TestBetterTv(t *testing.T) {
	...
	url1, err := emote_resolver.EmoteResolver.ResolveUrl("https://betterttv.com/emotes/5f1b0186cf6d2144653d2970")
	assert.True(err)
	assert.Equal(url1, "https://cdn.betterttv.net/emote/5f1b0186cf6d2144653d2970/2x")

	url2, err := emote_resolver.EmoteResolver.ResolveUrl("https://cdn.betterttv.net/emote/5f1b0186cf6d2144653d2970/3x.webp")
	assert.True(err)
...
func TestFrankerFaceZ(t *testing.T) {
	...
	url1, err := emote_resolver.EmoteResolver.ResolveUrl("https://www.frankerfacez.com/emoticon/128054-OMEGALUL")
	assert.True(err)
	assert.Equal(url1, "https://cdn.frankerfacez.com/emoticon/128054/2")

	url2, err := emote_resolver.EmoteResolver.ResolveUrl("https://cdn.frankerfacez.com/emoticon/128054/3")
	assert.True(err)
